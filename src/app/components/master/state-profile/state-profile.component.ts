...
export class StateProfileComponent implements OnInit {
  ...
  getPageDetils() {
    this.activeRoute.params.subscribe((params: Params) => {
      ...
      this.getDataByHash(splitLocation[0]).subscribe((response) => {
        console.log("response", response);
        this.currentLocation = response;
        this.townZip = response.zipcode;
        this.national_avg_price = response.nationalAvgPrice;
        this.totalCompanies = response.totalCompanies;
        this.fullserviceComapnies = response.fullserviceComapnies;
        this.discountComapnies = response.discountComapnies;
        this.logoUrls = response.logos;
        this.townImage_url = response.townImage;
  ...
  getDataByHash(slug: string): Observable<any> {
    let url = environment.api_url + `locations/hash?hash=${slug}`;
