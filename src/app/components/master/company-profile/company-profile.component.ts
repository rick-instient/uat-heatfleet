...
export class CompanyProfileComponent implements OnInit {
  ...
  GetCompanyProfile() {
    ...
    console.log("GetCompanyProfile", this.currentSlug);
    ...
    if (this.currentSlug) {
      ...
      // this.http.post(url, send).subscribe((data: any) => {
        ...
        data.nearByCompanies.forEach((element: any) => {
          ...
          if(element.logoURL){
            element.isDefaultLogo = false;
          }else{
            element.isDefaultLogo = true;
          }
        });
  ...
  QtyFocusOut(){
    this.quantityInput_withGal = (this.quantityInput_withGal === '' ? "100":this.quantityInput_withGal) + ' gal';
  }
